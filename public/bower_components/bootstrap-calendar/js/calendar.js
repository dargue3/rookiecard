...
Date.prototype.getWeek = function(self) {
	if(moment(this).week() < moment(self.options.first_event).week() - 1)
		return 0;
	else {
		var numWeeks = moment(this).week() - moment(self.options.first_event).week() + 1;
    return numWeeks;
  }
};
...
(function($) {
    ...
    first_event:         null,
		...
		modal:              null,
		//	modal handling setting, one of "iframe", "ajax", "vue", or "template"
		//dargue3: vue mode sets off a watcher bound to "data-event-id" on the given modal
		modal_type:         "vue",
	...
	Calendar.prototype.getTitle = function() {
		...
		switch(this.options.view) {
			...
			case 'month':
				...
				break;
			case 'week':
				return this.locale.title_week.format(p.getWeek(self), p.getFullYear());
				break;
			case 'day':
	...
	Calendar.prototype._update_modal = function() {
		...
		$('a[data-event-id]', this.context).on('click', function(event) {
			...
			if(!modal.data('handled.bootstrap-calendar') || (modal.data('handled.bootstrap-calendar') && modal.data('handled.event-id') != event.id)) {
				modal
					.unbind().on('show.bs.modal', function() {
	...
	Calendar.prototype._update_month = function() {
		...
		$('.cal-month-box .cal-row-fluid')
			.on('mouseenter', function() {
                ...
                var numWeeks = p.getWeek(self);
                if(numWeeks > 0) {
                    week.html(self.locale.week.format(numWeeks));
